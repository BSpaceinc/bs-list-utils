...
where Left: PartialEq,
  Right: PartialEq
{
  fn eq(&self, other: &Self) -> bool {
    match (self, other) {
      (DiffIgnored::Left(ref l), DiffIgnored::Left(ref r)) => {
        l == r
      },
      (DiffIgnored::Right(ref l), DiffIgnored::Right(ref r)) => {
        l == r
      },
      _ => false
    }
...
fn test_diff() {
  ...
  let l1: Vec<V1> = ["0", "a", "a", "a", "b", "b", "c"].iter().map(|v| V1(v.to_string())).collect();
  let l2: Vec<V2> = ["a", "a", "a", "b", "b", "c", "d"].iter().map(|v| V2(v)).collect();

  let res = diff(&l1, &l2);

  assert_eq!(res.left, vec![
    &V1("0".to_string())
  ]);
  assert_eq!(res.both, vec![
    (
      &V1("a".to_string()), &V2("a")
    ),
    (
